...
description: >-
  Vulkan graphics pipeline specialist for ClonStarCitizen. Use proactively for
  GLSL→SPIR-V shaders, vertex buffers, graphics pipelines, render pass / swapchain
  integration, and GPU buffer layout work.
---
...
1. Read existing `engine/vulkan/*` and render pass setup before changing APIs
2. Keep resources in `RendererState` (or adjacent POD structs); destroy at shutdown
3. Prefer push constants or a single init-time UBO updated in-place each frame
4. Verify with the `fast_compile` skill: `cmake --build build -j$(nproc)`
